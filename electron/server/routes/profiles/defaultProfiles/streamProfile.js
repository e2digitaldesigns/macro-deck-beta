...
const createStreamProfile = async () => {
  const database = await connectSQL();
  const profileName = "Streaming";
  const buttonPads = 12;
  ...
  try {
    ...
    await database.run(
      ...
      [
        ...
        [buttonPadId, profileId, pageId, 2],
        [buttonPadId, profileId, pageId, 3]
      ]
    );
...
module.exports.createStreamProfile = createStreamProfile;
